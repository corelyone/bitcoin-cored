...
static uint32_t GetBlockScriptFlags(const Config &config,
    ...
    // If the Core HF is enabled, we start rejecting transaction that use a high
...
static bool ConnectBlock(const Config &config, const CBlock &block,
    ...
    CExtDiskTxPos pos(CDiskTxPos(pindex->GetBlockPos(),
                      GetSizeOfCompactSize(block.vtx.size())), pindex->nHeight);
    ...
    for (size_t i = 0; i < block.vtx.size(); i++) {
        ...
        if (fAddrIndex) {
            if (!tx.IsCoinBase()) {
                for (const CTxIn &txin : tx.vin) {
                    const Coin &coin = view.AccessCoin(txin.prevout);
                    BuildAddrIndex(coin.GetTxOut().scriptPubKey, pos, vPosAddrid);
                }
            }
    ...
    if (fTxIndex && !pblocktree->WriteTxIndex(vPosTxid)) {
        return AbortNode(state, "Failed to write transaction index");
    }

    if (fAddrIndex && !pblocktree->AddAddrIndex(vPosAddrid)) {
